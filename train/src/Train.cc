...
/*
  example 
  --datafiles 1 test.txt --boardSize 9 --inputChannels 18 --inputFeatures 10 --residualBlocks 2 --residualChannels 32 --iterate 200 --batchSize 64 --learningRate 0.001 --weightDecay 0.001 --weightsName weights.txt --CUP_only

*/

constexpr static int DEFULT_INPUT_CHANNELS = 18;
...
struct ArgsParser {
  ...
  ArgsParser(int argc, char **argv) {
    ...
    if (input_channels == 0) {
      input_channels = DEFULT_INPUT_CHANNELS;
    }
    if (input_features == 0) {
      input_features = DEFULT_INPUT_FEATURES;
    }
  }
