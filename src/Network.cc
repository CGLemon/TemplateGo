...
void leelaz_process_bn_var(container& weights) {
...
std::pair<int, int> Network::load_leelaz_network(std::istream& wtfile) {
	...
	const int not_residual_lines = version_lines + tower_head_conv_lines + linear_lines;
...
void Network::initialize(int playouts, const std::string & weightsfile, Networkfile_t file_type) {
	...
	std::tie(channels, residual_blocks) = load_network_file(weightsfile, file_type);
	if (channels == 0) {
        exit(EXIT_FAILURE);
    }
...
std::vector<float> Network::gather_features(const GameState* const state,
    ...
    const auto moves = std::min<size_t>(state->board.get_movenum() + 1, INPUT_MOVES);
