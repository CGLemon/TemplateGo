...
#ifdef USE_CUDA
template <typename T>
__global__ void add_vectors_kernel(T *a, T *b, T *c, int asize, int bsize,
                                   int size, bool relu) {
...
template <typename T>
void cuda_add_vectors(T *a, T *b, T *c, int asize, int bsize,
                      int size, bool relu) {
...
__global__ void batchNorm_kernel(T *data, const float *means,
                                 const float *stddevs, int N, int C,
                                 int spatial, const T *eltwise, bool relu) {
...
void cuda_global_avg_pool(T *input, T *output, int batch, int channels, int spatial_size) {
  ...
  const int blocks = DivUp(size, kBlockSize);
  global_avg_pool_kernel<<<blocks, kBlockSize>>>(input, output, batch,
...
template void cuda_im2col<float>(int filter_size, int C, int H, int W,
                                 float *data_im, float *data_col);

template void cuda_global_avg_pool<float>(float *input, float *output,
                                          int batch, int channels, int spatial_size);

template void cuda_se_scale<float>(const float *input, const float *se_bias, float *data,
                                   int batch, int channels, int spatial_size);

template void cuda_input_pool<float>(const float *bias, float *data,
                                     int batch, int channels, int spatial_size);
...
template void cuda_add_vectors<float>(float *c, float *a, float *b, int size, int asize, int bsize, bool relu);
#endif
