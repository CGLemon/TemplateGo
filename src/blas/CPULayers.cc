...
void winograd_convolve3::sgemm(const std::vector<float> &U,
  ...
  for (int b = 0; b < WINOGRAD_TILE; b++) {
    ...
    blas::winograd_gemm(offset_u, offset_v, offset_m, K, P, C, 1.0f, U, K, V, P,
                        0.0f, M, P);
  }
}
...
void Convolve::im2col(const size_t filter_size, const int channels,
                      const std::vector<float> &input,
                      std::vector<float> &output) {
...
void Convolve::Forward(const size_t filter_size, const size_t outputs,
                       const std::vector<float> &input,
...
void Convolve_1::Forward(const size_t outputs, const std::vector<float> &input,
                         const std::vector<float> &weights,
                         const std::vector<float> &biases,
                         std::vector<float> &output) {
...
void Batchnorm::Forward(const size_t channels, std::vector<float> &data,
...
void FullyConnect::Forward(const int inputs, const int outputs,
                           const std::vector<float> &input,
                           const std::vector<float> &weights,
                           const std::vector<float> &biases,
...
static void apply_se(const size_t channels, const float *input,
                     const float *res, const float *scale, float *output) {
  ...
  for (auto c = size_t{0}; c < channels; c++) {
    ...
    for (auto i = size_t{0}; i < num_intersections; i++) {
      output[c * num_intersections + i] =
          lambda_ReLU(gamma * input[c * num_intersections + i] + beta +
