...
void CPUbackend::initialize(int channels, int residual_blocks, std::shared_ptr<ForwardPipeWeights> weights) {
...
void CPUbackend::forward(const std::vector<float>& input,
    ...
    Convolve_1::Forward(OUTPUTS_POLICY, conv_out, m_conv_pol_w, m_conv_pol_b, output_pol);
    Convolve_1::Forward(OUTPUTS_VALUE, conv_out, m_conv_val_w, m_conv_val_b, output_val);

}

void CPUbackend::push_weights(unsigned int /*filter_size*/,
